import pandas 
import download
import os #manipuler the paths


data_dir = "C:\Users\TRIUMPHAL\Exercices\weatherDenmark"
# Full path for the pickled data-file. (Original data).
#cette fontion retourne le chemain du pickle 
...
#retourne le chemain vers le fichier excel 
...
def _convert_raw_data(path):
    ...
    df = pd.DataFrame()#empty in the begining 

    # Get the city-name / weather-station name from the USAF code. : convertir le code en cité 
    ...
    df.set_index(['City', 'DateTime'], inplace=True)#notre index ici est la cité + the tame 
...
def _resample(df):
    ...
    # Upsample so the time-series has data for every minute. resample Rééchantillonne les données chronologiques. de 
    #sorte qu'on saute par miniyte 
    df_res = df_res.resample('1T')

    # Fill in missing values.
    df_res = df_res.interpolate(method='time')

    # Downsample so the time-series has data for every hour. on saute par heure 
...
def load_resampled_data():
    ...
    if os.path.exists(path):
        ...
        #nous devons nous mm le rassembler manuellement !
        ...
        df_cities = [df_org.xs(city) for city in cities] #que fait xs ? 
        #Retourne une section transversale (ligne (s) ou colonne (s)) 
        #à partir de Series / DataFrame. Par défaut, la section sur les lignes (axe = 0).
        #la clé ici est city qui est une partie de l'index!!! 
